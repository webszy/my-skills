...
快速流程：定位目标，向用户展示需求理解和影响面拆解，运行决策树进行初始分级，检查代码证据，再运行同一棵决策树进行最终分级，然后使用对应工作流。如果示例、决策树和分类条件冲突，采用最高风险等级。
...
Agent 只能在需求理解和需求拆解之后，对任务进行 Level S、Level M、Level L 或 Level XL 的初始分级。
