...
对于 Level M、Level L 和 Level XL 任务，Agent 必须先读取这份参考，再输出计划、设计或开始实现。对于 Level S 任务，读取这份参考是可选的，以保持简单改动足够轻量。
...
对于新模块、新服务、顶层工作流或架构设计，Agent 应进入设计流程，但仍要先快速搜索是否已有相同名称或相同目的的模块/工作流，避免重复建设。

这条规则适用于所有工作流等级，包括简单的 Level S 改动。它用于避免 Agent 猜测目标，或在用户要求“修改已有目标”时静默创建新目标。

## 基于证据的思考

定位目标后，Agent 必须先基于具体代码证据进行思考，再输出计划。
